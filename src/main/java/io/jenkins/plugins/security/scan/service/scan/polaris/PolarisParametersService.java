...
public class PolarisParametersService {
    ...
    public Polaris preparePolarisObjectForBridge(Map<String, Object> polarisParameters) {
        ...
        setContainerName(polarisParameters, polaris);
    ...
    private void setContainerName(Map<String, Object> polarisParameters, Polaris polaris) {
        if (polarisParameters.containsKey(ApplicationConstants.POLARIS_CONTAINER_NAME_KEY)) {
            Container container = polaris.getContainer() != null ? polaris.getContainer() : new Container();
            container.setName(polarisParameters
                    .get(ApplicationConstants.POLARIS_CONTAINER_NAME_KEY)
                    .toString()
                    .trim());
            polaris.setContainer(container);
        }
    }
    ...
    public Sarif prepareSarifObject(Map<String, Object> sarifParameters) {
        ...
        boolean shouldCreateSarifInDefaultPath =
                sarifParameters.containsKey(ApplicationConstants.POLARIS_REPORTS_SARIF_CREATE_KEY)
                        && !sarifParameters.containsKey(ApplicationConstants.POLARIS_REPORTS_SARIF_FILE_PATH_KEY);
        ...
        if (shouldCreateSarifInDefaultPath) {
            sarif.setFile(new File());
            sarif.getFile()
                    .setPath(ApplicationConstants.DEFAULT_POLARIS_SARIF_REPORT_FILE_PATH
